...
function prepare(rows){
  ...
  return{rows,close,sma5:smaSeries(close,5),sma10:smaSeries(close,10),sma25:smaSeries(close,25),sma50:smaSeries(close,50),sma200:smaSeries(close,200),ema21:emaSeries(close,21),ema65:emaSeries(close,65),rsi14:rsiWilder(close,14),atr14:atrWilder(rows,14)};
}
...
function relativeStrength(rows,benchmarkRows){
  ...
  if(aligned.length<21)return{ret20:null,benchmark_ret20:null,rs20:null};
  const end=aligned.at(-1),start=aligned.at(-21),r=pct(end.close,start.close),br=pct(byDate.get(end.date),byDate.get(start.date));
  return{ret20:round(r),benchmark_ret20:round(br),rs20:round(r-br)};
}
function setupState({daily,weekly,monthly,volume,rs}){
  const reasons=[],risks=[];let status='WAIT';
  const c=daily.close,ema=daily.ema65,s25=daily.sma25,s50=daily.sma50,rsi=daily.rsi14,vr=volume.ratio;
  ...
  else if(daily.trend.score>0&&weekly.trend.score>0&&nearBreak&&finite(vr)&&vr>=1.2&&finite(rs.rs20)&&rs.rs20>0){
...
export function analyzeFrame({symbol,name,market,rows,benchmarkSymbol,benchmarkRows,meta}){
  ...
  return{
    ...
    methodology:'月足・週足・日足の構造、EMA65、25/50/200日線、RSI14、ATR14、20日相対強度、20日出来高比を統合。売買注文は送信しません。'
  };
}
