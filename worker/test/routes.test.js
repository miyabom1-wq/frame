...
test('analyze route integrates fetch normalization and engine',async()=>{
  ...
  try{
    ...
    const d=await r.json();assert.equal(d.ok,true);assert.equal(d.symbol,'MU');assert.equal(d.benchmark,'^SOX');assert.ok(d.frames.daily);assert.ok(d.setup.entry);assert.ok(d.holding);assert.equal(d.setup.checklist.length,8);assert.equal(d.quote.price,d.frames.daily.close);assert.equal(d.quote.change_pct,d.frames.daily.change_pct);assert.equal(d.quote.date,d.frames.daily.date);
